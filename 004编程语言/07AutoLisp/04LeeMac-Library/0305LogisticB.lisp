...
(defun c:logisticb ( / *error* _PlotFunction dch r x xi yi ) (setq r 1.0 x 0.3)
  ...
  (defun _PlotFunction ( x r / f a b p )
    ...
    (start_image "b")
    (fill_image 1 1 (1- xb) (1- yb) 7)

    (setq p r r 0.01 x 0.3)
    (repeat (fix (* p 100))
      (repeat 20 (setq x (f x)))
      (repeat 10 (vector_image (setq a (fix (* r 100))) (setq b (- 200 (fix (* 200 (setq x (f x)))))) a b 5))
      (setq r (+ r 0.01) x 0.3)
    )    
    (end_image)
  )
  ...
  (cond
    ( (<= (setq dch (load_dialog "LogisticB.dcl")) 0)
    ...
    ( (setq xi (dimx_tile "f") yi (dimy_tile "f") xb (dimx_tile "b") yb (dimy_tile "b"))
