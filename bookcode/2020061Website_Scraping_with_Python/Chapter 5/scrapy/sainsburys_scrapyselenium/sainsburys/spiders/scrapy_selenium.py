...
class BasicSpider(Spider):
    ...
    def parse(self, response):
        ...
        for url in urls:
            if url.startswith('http'):
                yield SeleniumRequest(url=url, callback=self.parse_department_pages)
    ...
    def handle_product_listings(self, response):
        ...
        for url in urls:
            if url.startswith('http'):
                yield SeleniumRequest(url=url, callback=self.parse_product_detail)

        next_page = response.xpath('//ul[@class="pages"]/li[@class="next"]/a/@href').extract()
        if next_page:
            yield SeleniumRequest(url=next_page[0], callback=self.handle_product_listings)
