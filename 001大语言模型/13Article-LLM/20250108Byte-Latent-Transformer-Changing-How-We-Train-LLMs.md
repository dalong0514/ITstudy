...
[Byte Latent Transformer: Changing How We Train LLMs | AIGuys](https://medium.com/aiguys/byte-latent-transformer-changing-how-we-train-llms-b94924f1cc2e)
...
[[2412.09871] Byte Latent Transformer: Patches Scale Better Than Tokens](https://arxiv.org/abs/2412.09871?utm_source=chatgpt.com)
...
1 Single characters: ["u", "n", "h", "a", "p", "i", "e", "s"]

2 A special symbol to indicate subword continuation: ## (e.g., ##ness)

用特殊符号 ## 来表示子词的延续，例如 ##ness。

3 Common words or frequent subwords: ["happy", "un", "##ness"]
...
1 un is in the vocabulary, so it's selected.

'un' 这个词在词汇表中，因此被选中。

2 Remaining part: happiness.

剩余部分是：'happiness'。

Continue with the next part:

1 The next longest match is happy (from happiness).

接下来，最长的匹配是 happy（它来自 happiness）。

2 Remaining part: ness.
...
1 Character-Based Tokenization: SentencePiece works directly on raw text, bypassing language-specific preprocessing (e.g., no need to split the text into words).

基于字符的 Tokenization（Character-Based Tokenization)：SentencePiece 直接对原始文本进行处理，无需进行特定的语言预处理（例如，不需要将文本拆分成单词）。

2 Subword Units: It breaks text into subword units (like WordPiece) but doesn't rely on spaces as word boundaries.

子词（subword）单元：它将文本分解为子词单元（例如 WordPiece），但不依赖空格作为词的边界。

3 Model-Based Tokenization: Uses algorithms like Byte Pair Encoding (BPE) or Unigram Language Model for tokenization.
...
1 Characters: ["I", "l", "k", "t", "o", "e", "n", "c", "P", "S"]

2 Common subwords: ["I", "like", "to", "_learn", "Sentence", "Piece"]
...
1 Multilingual and Cross-Language Models (handling scripts from different languages in one model).

多语言和跨语言模型（Multilingual and Cross-Language Models)：在同一个模型中处理来自不同语言的文本。

2 Handling Rare, Special, or Out-of-Vocabulary Tokens (URLs, emojis, code snippets).

处理稀有、特殊或词汇表外的 Token（Token）：应对或识别 URL、表情符号、代码片段等在词汇表中不常见的特殊 Token。

3 Text Compression and Flexibility (fewer vocabulary requirements, but still retaining the ability to reconstruct the original input).

文本压缩和灵活性（降低词汇量的要求，但仍能完整还原原始输入）。

### 03. The Problem

Fig: Standard LLM training pipeline
...
功能：局部信息编码器处理原始字节数据，将它们转换为初步的局部特征表示，这些表示能够提取局部相关的上下文信息。
...
功能：局部解码器将处理后的图像块的特征表示转换回字节序列，从而实现准确的文本生成。
